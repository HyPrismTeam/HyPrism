...
| Источник загрузки | Откуда скачивать файлы игры | auto |

#### Варианты источника загрузки

| Значение | Описание |
|----------|----------|
| auto | Использовать официальный сервер, зеркало как резерв |
| official | Только официальные серверы Hytale (нужен аккаунт) |
| estrogen | Использовать зеркало EstroGen (licdn.estrogen.cat) |

Зеркало EstroGen предоставляет полные версии игры по адресу:
