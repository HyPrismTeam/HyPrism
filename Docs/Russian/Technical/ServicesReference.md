...
- **Источники:** Запрашивает списки версий с официального API Hytale и комьюнити зеркала (EstroGen)
- **Кэширование:** Списки версий кэшируются локально с настраиваемым TTL
- **Fallback на зеркало:** Если официальные серверы недоступны, автоматически используются версии с зеркала

### EstroGenVersionSource
- **Файл:** `Services/Game/Sources/EstroGenVersionSource.cs`
- **Назначение:** Источник версий для загрузки игры с комьюнити зеркала (licdn.estrogen.cat)
- **Формат URL:** `https://licdn.estrogen.cat/hytale/patches/{os}/{arch}/{branch}/0/{version}.pwr`
- **Патчи разницы:** `https://licdn.estrogen.cat/hytale/patches/{os}/{arch}/{branch}/{from}/{to}.pwr`
- **Парсинг HTML:** Использует regex для разбора nginx autoindex HTML для обнаружения версий
- **Валидация размера файла:** Версии с файлами < 1 МБ отфильтровываются (неполные загрузки)
- **Тест скорости:** Поддерживает тестирование скорости зеркала с измерением пинга и скорости загрузки
- **TTL кэша:** Кэш версий: 30 минут, Кэш теста скорости: 1 час
