...
Запускает консольное приложение .NET → создаёт нативное окно Sciter → загружает `wwwroot/index.html`.

### Разработка только фронтенда

```bash
cd Frontend
npm run dev    # Vite dev-сервер на localhost:34115
```

Полезно для итераций над UI без перезапуска всего приложения. Примечание: IPC-вызовы (`Window.this.xcall`) не будут работать в автономном режиме Vite — среда выполнения Sciter недоступна в браузере. Для разработки можно замокировать `Window.this.xcall` в `ipc.ts`.
