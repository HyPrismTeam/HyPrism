...
import {  X, RefreshCw, Check, User, Edit3, Copy, CheckCircle, Plus, Trash2, Dices, FolderOpen, CopyPlus, Lock  } from '../components/icons/LucideIcons';
...
export const ProfileEditor: React.FC<ProfileEditorProps> = ({ isOpen, onClose, onProfileUpdate, pageMode: isPageMode = false }) => {
    ...
    const handleUsernameKeyDown = (e: React.KeyboardEvent<HTMLElement>) => {
        if (e.key === 'Enter') {
            handleSaveUsername();
    ...
    const handleUuidKeyDown = (e: React.KeyboardEvent<HTMLElement>) => {
        if (e.key === 'Enter') {
            handleSaveUuid();
        } else if (e.key === 'Escape') {
            setEditUuid(uuid);
    ...
    const handleDeleteProfile = (profileId: string, e: React.MouseEvent<HTMLElement>) => {
        e.stopPropagation();
        e.preventDefault();
    ...
    const handleDuplicateProfile = async (profileId: string, e: React.MouseEvent<HTMLElement>) => {
        e.stopPropagation();
        try {
                                            ...
                                            {isEditingUsername ? (
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        type="text"
                                                        value={editUsername}
                                                        onChange={(e) => setEditUsername((e.target as HTMLInputElement).value)}
                                        ...
                                        {isEditingUuid ? (
                                            <div className="flex items-center gap-2">
                                                <input
                                                    type="text"
                                                    value={editUuid}
                                                    onChange={(e) => setEditUuid((e.target as HTMLInputElement).value)}
