...
const _hasElectron = typeof _w.require === 'function';
const _hasTauriGlobal = !!_w.__TAURI__?.core?.invoke;
const _hasTauriInternals = !!_w.__TAURI_INTERNALS__?.invoke;

// eslint-disable-next-line @typescript-eslint/no-require-imports
const ipcRenderer = _hasElectron ? _w.require('electron').ipcRenderer : null;
const tauriInvoke = _hasTauriGlobal
  ? _w.__TAURI__.core.invoke
  : (_hasTauriInternals ? _w.__TAURI_INTERNALS__.invoke : null);
const tauriListen = _hasTauriGlobal ? _w.__TAURI__.event.listen : null;
const tauriUnlisten = _hasTauriGlobal ? _w.__TAURI__.event.unlisten : null;
...
export function send(channel: string, data?: unknown): void {
  ...
  if (tauriInvoke) {
    void tauriInvoke('bridge_send', { channel, payload: data ?? null });
    return;
  }

  throw new Error('No IPC runtime detected (Electron/Tauri)');
}
...
export function on(channel: string, callback: (data: unknown) => void): () => void {
  ...
  if (tauriListen && tauriUnlisten) {
    let disposed = false;
    let listenerId: number | null = null;

    void tauriListen('hyprism:bridge:event', (event: any) => {
      const envelope = event?.payload;
      if (envelope?.channel !== channel) return;
      callback(envelope.payload);
    }).then((id: number) => {
      if (disposed) {
        void tauriUnlisten(id);
      } else {
        listenerId = id;
      }
    });
  ...
  if (tauriInvoke) {
    ...
    window.addEventListener('hyprism:bridge:event', domListener as EventListener);
    return () => window.removeEventListener('hyprism:bridge:event', domListener as EventListener);
  }
...
export function invoke<T = unknown>(channel: string, data?: unknown, timeout = 10000): Promise<T> {
  if (tauriInvoke) {
    return tauriInvoke('bridge_invoke', { channel, payload: data ?? null, timeoutMs: timeout }) as Promise<T>;
  }

  if (!ipcRenderer) {
    return Promise.reject(new Error('No IPC runtime detected (Electron/Tauri)'));
  }
