...
// Sciter runtime provides Window as a global — declare it for TypeScript
declare const Window: { this: { xcall(name: string, ...args: unknown[]): unknown } };

// Listener registry for C#→JS push events (__hyprismReceive)
const _listeners: Record<string, Array<(data: unknown) => void>> = {};

// Called by C# backend: globalThis.__hyprismReceive(channel, jsonString)
(globalThis as any).__hyprismReceive = (channel: string, json: string): void => {
  const cbs = _listeners[channel];
  if (!cbs || cbs.length === 0) return;
  let data: unknown = json;
  if (typeof json === 'string') {
    try { data = JSON.parse(json); } catch { /* keep raw string */ }
  }
...
export function send(channel: string, data?: unknown): void {
  Window.this.xcall('hyprismCall', channel, data != null ? JSON.stringify(data) : '');
}
