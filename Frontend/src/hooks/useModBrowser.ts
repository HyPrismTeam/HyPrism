...
export const useModBrowser = (options: UseModBrowserOptions) => {
  ...
  const handleOpenModPage = useCallback((e: React.MouseEvent<HTMLElement>, mod: ModInfo) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleBrowseShiftLeftClick = useCallback((e: React.MouseEvent<HTMLElement>, index: number) => {
