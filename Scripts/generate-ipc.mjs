...
// 1. Sciter xcall transport + listener registry
L.push(`// Sciter runtime provides Window as a global — declare it for TypeScript`);
L.push(`declare const Window: { this: { xcall(name: string, ...args: unknown[]): unknown } };`);
L.push(``);
L.push(`// Listener registry for C#→JS push events (__hyprismReceive)`);
L.push(`const _listeners: Record<string, Array<(data: unknown) => void>> = {};`);
L.push(``);
L.push(`// Called by C# backend: globalThis.__hyprismReceive(channel, jsonString)`);
L.push(`(globalThis as any).__hyprismReceive = (channel: string, json: string): void => {`);
L.push(`  const cbs = _listeners[channel];`);
L.push(`  if (!cbs || cbs.length === 0) return;`);
L.push(`  let data: unknown = json;`);
L.push(`  if (typeof json === 'string') {`);
L.push(`    try { data = JSON.parse(json); } catch { /* keep raw string */ }`);
L.push(`  }`);
L.push(`  for (const cb of cbs) cb(data);`);
L.push(`};`);
L.push(``);
...
L.push(`  Window.this.xcall('hyprismCall', channel, data != null ? JSON.stringify(data) : '');`);
L.push(`}`);
L.push(``);
L.push(`export function on(channel: string, callback: (data: unknown) => void): () => void {`);
L.push(`  if (!_listeners[channel]) _listeners[channel] = [];`);
L.push(`  _listeners[channel].push(callback);`);
L.push(`  return () => {`);
L.push(`    const arr = _listeners[channel];`);
L.push(`    if (arr) {`);
L.push(`      const idx = arr.indexOf(callback);`);
L.push(`      if (idx >= 0) arr.splice(idx, 1);`);
L.push(`    }`);
L.push(`  };`);
L.push(`}`);
L.push(``);
