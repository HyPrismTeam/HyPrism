...
// Parse @ipc annotations (optional input type: @ipc invoke channel [{ id: string }] -> Type; optional timeout)
const ipcRe = /\/\/\s*@ipc\s+(invoke|send|event)\s+(hyprism:\S+)(?:\s+\[([^\]]+)\])?(?:\s*->\s*(.+?))?(?:\s+(\d+))?\s*$/gm;

/** @type {{ type: string, channel: string, inputType?: string, responseType?: string, timeout?: number }[]} */
const channels = [];
let im;
while ((im = ipcRe.exec(source)) !== null) {
  channels.push({
    type: im[1],
    channel: im[2],
    inputType: im[3]?.trim() || undefined,
    responseType: im[4]?.trim() || undefined,
    timeout: im[5] ? parseInt(im[5], 10) : undefined,
  });
}
...
L.push(`        .-. :                               `);
L.push(`        \`._.'             generate-ipc.mjs`);
L.push(``);
L.push(`AUTO-GENERATED — DO NOT EDIT MANUALLY!`);
...
for (const [domain, chs] of Object.entries(domains)) {
  ...
  for (const ch of chs) {
    ...
    if (ch.type === 'invoke') {
      ...
      } else if (ch.inputType) {
        L.push(`  ${action}: (data: ${ch.inputType}) => invoke<${resp}>('${ch.channel}', data${timeoutArg}),`);
      } else {
